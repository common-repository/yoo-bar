        ...
        while ($args->have_posts())
        {
           ...
           include YOO_BAR_PATH . 'assets/public/extra/ToPviews.php';
            ...
            $display_footer_bar = get_post_meta($post_id, "display_footer_bar", true);
                ...
                $yooslideUp = 'yoobarfooterRand';
          

            $yoo_bar_fixedfooterbar = get_post_meta($post_id, "yoo_bar_fixedfooterbar", true);
            if ($yoo_bar_fixedfooterbar)
            {
                $yoo_bar_fixedfooterbar = 'yoobarstickynav';
            }
            else
            {
                $yoo_bar_fixedfooterbar = ' ';
            }
            if ($display_footer_bar)
            {

                $html = '<style>.yoo-conatainer-' . $rand . '{
                position:relative;z-index:1
                }.yoo-conatainer-' . $rand . ':before {
                background:' . $yoo_bar_back . ';z-index:-1;content:"";position:absolute;width:100%;height:100%;left:0;}</style><div id="topbarpositong" class="' . $yooslideUp . '"><div id="alert-container" class="' . $yoo_bar_fixedfooterbar . '">
...
if (class_exists('Yoobar_Footer_bar'))
{
    new Yoobar_Footer_bar;
};
